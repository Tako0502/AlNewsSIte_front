window.$qar = {
    ...
    getCurrentLanguage: ()=>{
        let pathName = window.location.pathname.startsWith("/")?window.location.pathname.substring(1):window.location.pathname,
              pArr = pathName?pathName.split('/'):[],
              currentLanguge = pArr.length>0 ?pArr[0].toLowerCase():'';
        return $qar.siteLanguges.includes(currentLanguge)?currentLanguge:$qar.defaultLanguage;
    },
    setBtnStatus: (btn,status) => {
        if(status === 'loading'){
            btn.setAttribute('data-old-html',btn.innerHTML);
            btn.innerHTML = `<span class="spinner-border spinner-border-sm" aria-hidden="true"></span>
                             <span role="status">`+btn.getAttribute('data-loading-text')+`</span>`;
             btn.disabled = true;
    ...
    removeFormInvalidFeedback: (form)=>{
        const isInvalidElements  = form.querySelectorAll('.is-invalid'),
              ...
              isInvalidElements.forEach(element => element.classList.remove("is-invalid"));
    ...
    showFormInputError: (input,message) => {
        const form = input.closest('form');
         ...
         input.focus();
         input.addEventListener('change',()=>{$qar.removeFormInvalidFeedback(form);console.log('change'); },{ once: true });
         input.addEventListener('keyup',()=>{$qar.removeFormInvalidFeedback(form);console.log('keyup'); },{ once: true });
         input.addEventListener('input',()=>{$qar.removeFormInvalidFeedback(form); console.log('input'); },{ once: true });
    },
    ...
    removeCustomModal:()=>{
        if(document.querySelector('#qar-custom-modal')){
            if(document.querySelector('#qar-custom-modal').classList.contains('show') && $qar.qarModal){
                $qar.qarModal.hide(); 
                $qar.qarModal = null;
                return;
            }
    ...
    showCustomModal:async (url,width)=>{
                ...
                const res = await fetch(url,{ method: 'GET'} ),
                 ...
                 qarModalNode.innerHTML = `<div class="modal-dialog"><div class="modal-content">`+resData+`</div></div>`;
                 if(width && !isNaN(width)){
                    qarModalNode.querySelector('div.modal-dialog').style.width = width+'px';
                    qarModalNode.querySelector('div.modal-dialog').style.maxWidth = width+'px';
                 }
          $qar.qarModal = new bootstrap.Modal(qarModalNode, {"backdrop":true,"keyboard":true,"focus":true});
           qarModalNode.addEventListener('shown.bs.modal', function (event) {
               document.querySelector('#qar-custom-modal').querySelectorAll('script').forEach(originalScript => {
    ...
    getDataTableLanguage: () => {
        ...
        return {
            "sSearch": dataTableLanguage.querySelector('div[data-key="sSearch"]').innerHTML,
            "sProcessing": dataTableLanguage.querySelector('div[data-key="sProcessing"]').innerHTML,
            "sLengthMenu": dataTableLanguage.querySelector('div[data-key="sLengthMenu"]').innerHTML,
            "sZeroRecords": dataTableLanguage.querySelector('div[data-key="sZeroRecords"]').innerHTML,
            "sInfo": dataTableLanguage.querySelector('div[data-key="sInfo"]').innerHTML,
            "sInfoEmpty": dataTableLanguage.querySelector('div[data-key="sInfoEmpty"]').innerHTML,
            "sInfoFiltered": dataTableLanguage.querySelector('div[data-key="sInfoFiltered"]').innerHTML,
            "sInfoPostFix": dataTableLanguage.querySelector('div[data-key="sInfoPostFix"]').innerHTML,
            "sUrl": dataTableLanguage.querySelector('div[data-key="sUrl"]').innerHTML,
            "oPaginate": {
                "sFirst": dataTableLanguage.querySelector('div[data-key="oPaginate"] > div[data-key="sFirst"]').innerHTML,
                "sPrevious": dataTableLanguage.querySelector('div[data-key="oPaginate"] > div[data-key="sPrevious"]').innerHTML,
                "sNext": dataTableLanguage.querySelector('div[data-key="oPaginate"] > div[data-key="sNext"]').innerHTML,
                "sLast": dataTableLanguage.querySelector('div[data-key="oPaginate"] > div[data-key="sLast"]').innerHTML
            }
        };
    },
    ...
    bindTinymceEditor: function bindTinymceEditor() {
        ...
        document.querySelectorAll('.tinymce-editor').forEach(function (v, i) {
            ...
            tinymce.init({
                ...
                statusbar: false,
                menubar: false,
                plugins: 'inputkzchar link image lists charmap preview anchor pagebreak searchreplace wordcount visualblocks visualchars code fullscreen insertdatetime media nonbreaking table directionality emoticons',
                toolbar: [
                ...
                setup: function (editor) {
                    ...
                    editor.ui.registry.addButton('customImage', {
                        ...
                        onAction: function () {
                            ...
                            $qar.showCustomModal("/" + $qar.getCurrentLanguage() + "/Modal/UploadEditorImage");
                        }
                    });
                    ...
                    editor.ui.registry.addButton('customUpload', {
                        ...
                        onAction: function () {
                            ...
                            $qar.showCustomModal("/" + $qar.getCurrentLanguage() + "/Modal/UploadEditorFile");
                        }
                    });
                    ...
                    editor.ui.registry.addButton('customSocial', {
                        ...
                        onAction: function () {
                            ...
                            $qar.showCustomModal("/" + $qar.getCurrentLanguage() + "/Modal/EditorSocialEmbedcode");
                        }
                    });
                },
                relative_urls: false,
